...
Upload() {
    ...
    do 
        sleep 3
        CODE_ID=$(injectived query tx $UPLOADTX $NODECHAIN --output json | jq -r '.logs[0].events[-1].attributes[-1].value')
    done
...
InstantiateIncentive() {
    ...
    TXHASH=$(injectived tx wasm instantiate $CODE_INCENTIVE '{}' --label "Incentive$CODE_INCENTIVE" --amount 5000000ustars --admin $ADDR_ADMIN $WALLET $TXFLAG -y --output json | jq -r '.txhash')
